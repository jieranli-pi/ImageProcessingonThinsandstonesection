...
for i in range(0,num_row):
    for j in range(0,num_col):
        Combined_image[int(mypath_array[ct].partition('x')[2].partition('x')[0]):int(mypath_array[ct].partition('x')[2].partition('x')[0])+a,\
                       int(mypath_array[ct].partition('x')[0]):int(mypath_array[ct].partition('x')[0])+b,\
                       :]\
        =np.load(input_directory_array+mypath_array[ct]).astype(np.uint8)
        ct+=1
